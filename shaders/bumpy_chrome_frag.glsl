...
{	
	...
	vec4 env_map_color = texture2D(env_map, gl_TexCoord[1].st);
	...
	vec4 ambient_global = gl_LightModel.ambient * gl_FrontMaterial.ambient * env_map_color;
	
	// The ambient component from light 0
	vec4 ambient = gl_FrontMaterial.ambient * gl_LightSource[0].ambient * env_map_color;
	...
	vec4 diffuse = gl_FrontMaterial.diffuse * gl_LightSource[0].diffuse * env_map_color * NdotL * att;
